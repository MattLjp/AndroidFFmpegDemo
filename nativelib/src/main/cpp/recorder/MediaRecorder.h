...
private:
    ...
    //添加媒体流程
    void
    AddStream(AVOutputStream *ost, AVFormatContext *oc, const AVCodec **codec, AVCodecID codec_id);

    //打印 packet 信息
    void PrintfPacket(AVFormatContext *fmt_ctx, AVPacket *pkt);

    //打开音频编码器
    int OpenAudio(AVFormatContext *oc, const AVCodec *codec, AVOutputStream *ost);

    //打开视频编码器
    int OpenVideo(AVFormatContext *oc, const AVCodec *codec, AVOutputStream *ost);
...
private:
    ...
    const AVOutputFormat *m_OutputFormat = nullptr;
