package com.matt.nativelib.media.render.egl
...
class CustomerGLRenderer : SurfaceHolder.Callback {
    ...
    private val mRenders = mutableListOf<OpenGlRender>()
    ...
    inner class RenderThread : Thread() {
        ...
        private fun initEGL() {
            ...
            mEGLSurface?.init(null, EGLCore.EGL_RECORDABLE_ANDROID)
        }
        ...
        private fun generateTextureID() {
            val textureIds = OpenGLTools.createTextureIds(mRenders.size)
            for ((idx, drawer) in mRenders.withIndex()) {
                drawer.surfaceCreated(textureIds[idx])
            }
        }

        private fun configWordSize() {
            mRenders.forEach { it.surfaceChanged(mWidth, mHeight) }
        }
